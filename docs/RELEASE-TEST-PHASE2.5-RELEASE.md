...
**버전**: v3.1.0  
**최종 업데이트**: 2025-11-10 00:00:00 KST  
...
### ⚠️ 운영 환경 주의사항
- **운영 환경**: `reshow_interval_seconds = NULL`, `reshow_interval_minutes = NULL` (항상!)
- **테스트 환경**: 빠른 테스트를 위해 초/분 단위 사용 가능
...
| S2 첫 "나중에" 클릭 | 시간 추적 시작 | 팝업 닫힘, 재시작 시 미표시 (24시간 미경과) |
| S3 시간 경과 후 재표시 | 24시간 후 재표시 로직 | 팝업 재표시, count 증가 |
...
**중요**: 모든 업데이트 관련 로그는 `HomeScreen` 태그로 출력됩니다 (UpdateDialog, UpdateManager 태그 없음)
...
- 첫 "나중에": `HomeScreen: Update dialog dismissed for code=10`
- 시간 미경과: `HomeScreen: ⏸️ Update dialog skipped (within reshow interval)`
- 시간 경과: `HomeScreen: ⏱️ Update interval elapsed (>= 1h), reshow allowed`
- 3회 도달: `HomeScreen: 🚨 Later count (3) >= max (3), forcing update mode`
- 강제 전환: `HomeScreen: Decision: FORCE UPDATE from update_policy (target=10)`
...
```sql
-- 디버그 행 존재 여부 자동 확인 후 INSERT 또는 UPDATE
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM update_policy WHERE app_id = 'com.sweetapps.pocketchord.debug') THEN
        -- 행이 있으면 UPDATE
        UPDATE update_policy
        SET is_active = true,
...
**기대 결과**:
- debug 행 존재, target=10, force=false, active=true, seconds=30, max=3
...
**전제조건**: 섹션 3의 SQL을 이미 실행했음 (필드 추가 + 초기값 설정)
...
**기대 결과**: 
- 4개 행 반환 (max_later_count, reshow_interval_hours/minutes/seconds)
...
| com.sweetapps.pocketchord.debug     | true      | 10                  | false           | 1                    | NULL                   | 30                     | 3               |
```
...
**전제조건**: S1 완료 (DB 필드 추가 및 초기값 설정이 완료된 상태)

**대상**: 디버그 앱 (reshow_interval_seconds = 30초)
...
**기대 결과**: 
- target=10, force=false, active=true
...
**✅ 사용자의 로그가 위와 동일합니다! "나중에" 클릭이 정상 처리되었습니다!**
...
- `update_later_count`: 1
- `dismissedVersionCode`: 10

---

**4단계: 재시작 후 미표시 확인 (1시간 이내)**
...
**참고**: 
- **1시간 경과 후**에는 `dismissedVersionCode`를 무시하고 재표시됨 (S3에서 테스트)
...
**대상**: 디버그 앱 (reshow_interval_hours = 1시간)
...
2. **실제로 30초 대기** (디버그 앱 기준 - `reshow_interval_seconds = 30`)
   - 또는 에뮬레이터 시스템 시간을 30초 앞으로 변경:
   ```cmd
   adb -s emulator-5554 shell su root date @$(($(($(date +%s) + 30))))
   ```
...
HomeScreen: ===== Phase 2: Trying update_policy =====
HomeScreen: ✅ update_policy found: targetVersion=10, isForce=false
HomeScreen: ⏱️ Update interval elapsed (>= 30s), reshow allowed  ← ✅ 시간 경과 확인!
HomeScreen: 🔄 Later count: 1 → 2  ← ✅ 카운트 증가!
HomeScreen: Decision: OPTIONAL UPDATE from update_policy (target=10)
HomeScreen: ===== Popup Display Check =====
HomeScreen: showEmergencyDialog: false
...
3. ✅ `⏱️ Update interval elapsed (>= 30s), reshow allowed` (시간 경과 감지!)
4. ✅ `🔄 Later count: 1 → 2` (카운트 자동 증가!)
5. ✅ `Decision: OPTIONAL UPDATE` (선택적 업데이트 결정)
6. ✅ `showUpdateDialog: true` (팝업 재표시)
...
2. 실제 경과 시간 확인:
```kotlin
// 현재 시간 - dismissedTime >= reshowIntervalMs (30000ms = 30초)
```
...
```
HomeScreen: Update dialog dismissed for code=10
HomeScreen: ⏱️ Tracking: laterCount=1→2, timestamp=1731150000000  ← ✅ 카운트 증가 추적!
HomeScreen: ===== Popup Display Check =====
HomeScreen: showUpdateDialog: false
HomeScreen: showAnnouncementDialog: false
```
...
- ✅ `🔄 Later count: 1 → 2` 로그 확인 (카운트 증가)
- ✅ "나중에" 클릭 후 `laterCount=1→2` 추적 로그 확인
...
HomeScreen: ===== Phase 2: Trying update_policy =====
HomeScreen: ✅ update_policy found: targetVersion=10, isForce=false
HomeScreen: ⏱️ Update interval elapsed (>= 30s), reshow allowed
HomeScreen: 🚨 Later count (3) >= max (3), forcing update mode
HomeScreen: Decision: FORCE UPDATE from update_policy (target=10)
HomeScreen: ===== Popup Display Check =====
...
SET target_version_code = 10,
    ...
    reshow_interval_seconds = 30,    -- 30초 (초고속 테스트!)
    max_later_count = 3,
    is_active = true
WHERE app_id = 'com.sweetapps.pocketchord.debug';
...
SET target_version_code = 10, is_force_update = false,
    reshow_interval_hours = 1, reshow_interval_minutes = NULL, reshow_interval_seconds = 30,
...
**문서 버전**: v3.1.0  
**마지막 수정**: 2025-11-10 00:15:00 KST
