# PocketChord Supabase 통합 완전 가이드

> PocketChord 앱에서 Supabase를 활용하여 공지사항, 앱 업데이트 정보, 배너 설정 등 서버 관리 콘텐츠를 구현하는 완전한 가이드입니다.
...
- **배너 광고 설정**: AdMob 광고 단위 ID, 활성화 여부 원격 제어
- **이벤트 정보**: 한정 이벤트, 프로모션
...
VALUES 
  ('환영합니다!', 'PocketChord에 오신 것을 환영합니다.', 'info', 10, true),
  ('새로운 기능', '이제 더 많은 코드를 볼 수 있습니다.', 'update', 5, true);
...
-- 테스트 배너 설정 추가
INSERT INTO banner_config (location, ad_unit_id, is_enabled)
VALUES ('home', 'ca-app-pub-3940256099942544/6300978111', true);
```
...
**위치**: `app/src/main/java/com/sweetapps/pocketchord/data/supabase/SupabaseModels.kt`
...
class SupabaseRepository(private val supabase: SupabaseClient) {
     ...
     * 특정 위치의 배너 설정 조회
     */
    suspend fun getBannerConfig(location: String): Result<BannerConfig?> = runCatching {
        supabase.from("banner_config")
            .select()
...
1. **Table Editor** → `banner_config` 선택
2. 해당 행의 `is_enabled` 수정
   - `true` → 광고 표시
   - `false` → 광고 숨김
3. 앱 재시작 시 즉시 반영
...
### Supabase 설정
- [ ] Supabase 프로젝트 생성
- [ ] 테이블 생성 (announcements, app_versions, banner_config)
- [ ] RLS 정책 설정
- [ ] 테스트 데이터 추가
- [ ] API 키 확인
